   ...
   '''
    ...
    def trim_stop_codons(self, gc=None, allow_partial=False, **kwargs):
        ...
        for seq_name in self.names:
            ...
            if not aligned:
                new_seq = old_seq.without_terminal_stop_sodon(gc=gc,
                                                           allow_partial=allow_partial)
                new_seqs.append((seq_name, new_seq))
                continue

            new_seq = old_seq.data.without_terminal_stop_sodon(gc=gc,
                                                            allow_partial=allow_partial)
...
class DenseAlignment(AlignmentI, SequenceCollection):
    ...
    def filtered(self, predicate, motif_length=1, **kwargs):
        ...
        shaped = self.array_seqs.reshape((self.num_seqs, num_motifs, motif_length))
        indices = []
