...
class DictArrayTest(TestCase):
    ...
    def test_convert2DDict(self):
        ...
        assert set(row_keys) == {"a"}
        b = DictArrayTemplate(row_keys, col_keys).wrap(vals)
        assert b.array.tolist() == [[4, 5]]
        # row keys, then column
        assert b.template.names == [["a"], ["b", "c"]]
    ...
    def test_convert2Ddistance(self):
        ...
        b = DictArrayTemplate(row_keys, col_keys).wrap(vals)
        assert b.array.tolist() == [[0, 4, 5], [4, 0, 0], [5, 0, 0]]
    ...
    def test_construct_both_dim_str(self):
        ...
        assert b[0].array.tolist() == [1, 0, 0]
        assert b["a"].array.tolist() == [1, 0, 0]
        assert b.array.tolist() == [[1, 0, 0], [0, 1, 0], [0, 0, 1]]

    def test_key_levels(self):
        """DictArray both levels have keys."""
        b = DictArrayTemplate("abc", "ABC").wrap(self.a)
        assert b.keys() == ["a", "b", "c"]
        assert b["a"].keys() == ["A", "B", "C"]
        assert list(b["a"]) == [1, 0, 0]
        assert sum(b["a"]) == 1
    ...
    def test_to_dict_1d(self):
        ...
        assert darr.to_dict(flatten=True) == expect

        darr = DictArrayTemplate(["s1", "s2"], ["s1", "s2"]).wrap(
            [[0.0, 0.25], [0.25, 0.0]],
        )
        assert darr.to_dict(flatten=True) == {
            ("s1", "s2"): 0.25,
            ("s2", "s1"): 0.25,
            ("s1", "s1"): 0.0,
            ("s2", "s2"): 0.0,
    ...
    def test_to_dict_nested(self):
        ...
        assert isinstance(c.to_dict()["d"], dict)
    ...
    def test_to_dict_roundtrip(self):
        ...
        d1 = {"a": {"k": 1, "l": 2, "m": 3}, "b": {"k": 4, "l": 5, "m": 6}}
        darr1 = DictArray(d1)
        d2 = darr1.to_dict()
        assert d1 == d2
        d3 = DictArray(d2)
    ...
    def test_convert_for_dictarray(self):
        ...
        assert got.array.tolist() == b.array.tolist()
        # the wrap method creates a new array
        assert got.array is not b.array
    ...
    def test_to_table(self):
        ...
        t = a2D.to_table()
        assert isinstance(t, Table)
        assert t.shape == (2, 3)  # because index_name column added
        # make sure the 2D variant has an index_name column, name is empty string
        assert t.index_name == ""
        assert t.columns[""].tolist() == a2D.template.names[0]
        # which works
        assert t["b", "d"] == 3
    ...
    def test_add(self):
        ...
        assert darr3.template.names == darr1.template.names
