...
ece:Z4182	cvi:CV2422	41.67	72	42	0	39	110	29	100	2e-06	52.8""".split(
    ...
    def test_iter_finder(self):
        ...
        lines = "abc\n# Iteration: 3\ndef".splitlines()
        assert list(map(iter_finder, lines)) == [False, True, False]

    def test_query_finder(self):
        """query_finder should split on lines starting with '# Query:'"""
        lines = "abc\n# Query: dfdsffsd\ndef".split("\n")
        assert list(map(query_finder, lines)) == [False, True, False]

    def test_iteration_set_finder(self):
        """iter_finder should split on lines starting with '# Iteration:'"""
        lines = "abc\n# Iteration: 3\ndef\n# Iteration: 1".split("\n")
        assert list(map(iteration_set_finder, lines)) == [False, False, False, True]
    ...
    def test_is_junk(self):
        ...
        lines = "abc\n# BLAST blah blah\n   \n# BLAT blah\n123".split("\n")
        assert list(map(is_blast_junk, lines)) == [False, True, True, False, False]
        assert list(map(is_blat_junk, lines)) == [False, False, True, True, False]
    ...
    def test_make_label(self):
        ...
        assert make_label(c) == ("ITERATION", 1)
        assert make_label(d) == (
            "QUERY",
            "ece:Z4147  ygdP; putative invasion protein [EC:3.6.1.-]",
        )
        assert make_label(e) == ("ITERATION", 1)
    ...
    def test_TableToValues(self):
        ...
        assert TableToValues(table, constructors) == (
            [["1.5", 3.5, 2, "2.5"], ["1", 2.0, 3, "4"]],
            ["c", "b", "a", "d"],
        )
        # check that it works with supplied header
        assert TableToValues(table[1:], constructors, list("cbad")) == (
            [["1.5", 3.5, 2, "2.5"], ["1", 2.0, 3, "4"]],
            ["c", "b", "a", "d"],
        )
    ...
    def test_PsiBlastParser9(self):
        ...
        assert len(iter_2) == 7
        iter_1_2 = second[0]
        assert len(iter_1_2) == 3
        assert len(result["ece:Z4181"][1][3]) == 12
        assert result["ece:Z4181"][1][3]["ALIGNMENT LENGTH"] == 103
    ...
    def test_LastProteinIds9(self):
        ...
        assert result == ["ece:Z4181", "ecs:ECs3717", "cvi:CV2421", "sfl:CP0138"]
        ...
        assert result[1] == ["ece:Z4182", "ecs:ECs3718", "cvi:CV2422"]
...
""".splitlines()
        ...
        assert len(recs) == 2
        assert recs[0] == text[:5]  # includes trailing blank
...
""".splitlines()
        ...
        r0, r1 = recs
        assert r0["tax_id"] == "3349"
        assert r0["common_name"] == "Scots pine"
        assert r0["scientific_name"] == "Pinus sylvestris"
        assert r0["seq_id"] == "gi|3021565|emb|AJ223314.1|PSAJ3314"
        assert r1["tax_id"] == "228610"
