---
title: "Evolutionary Analysis Using Likelihood"
---
...
A more extensive description of capabilities is in [Allowing substitution model parameters to differ between branches](../examples/scope_model_params_on_trees.qmd).
...
For more detailed discussion of defining and using these models see [Analysis of rate heterogeneity](../examples/rate_heterogeneity.qmd).
...
For more detailed discussion of defining and using these models see [Evaluate process heterogeneity using a Hidden Markov Model](../examples/hmm_par_heterogeneity.qmd).
...
If we can't rely on the asymptotic behaviour of the LRT, e.g. due to small alignment length, we can use a parametric bootstrap. Convenience functions for that are described in more detail here [Performing a parametric bootstrap](../examples/parametric_bootstrap.qmd).
...
The best approach is to use the json string from the `to_json()` method. The saved data can be later reloaded using `scinexus.deserialise.deserialise_object()`. The `json` data contains the alignment, tree topology, substitution model, parameter values, etc..
