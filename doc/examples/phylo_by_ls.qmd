---
title: "Phylogenetic reconstruction by least squares"
---
We will load some pre-computed pairwise distance data. To see how that data was computed see the [Calculate pairwise distances between sequences](calculate_pairwise_distances.qmd) example. That data is saved in a format called `pickle` which is native to python. As per usual, we import the basic components we need.
