...
# ******NOTICE***************
# based on SciPy optimize.py 0.5 by Travis E. Oliphant
#
...
"""based on scipy's optimize.py

A collection of general-purpose optimization routines using numpy
...
brute       ---      Perform a brute force search for the minimum
                       with final optimization if desired.  BROKEN
...
def atleast_1d(x):
    if not hasattr(x, 'shape'):
        x = Num.array([x])
    return x
# which leaves just mgrid and vectorize, but they are only
# needed for brute force anyway.

def vecnorm(x, ord=2):
    if ord == Inf:
        return Num.maximum.reduce(abs(x))
    elif ord == -Inf:
        return Num.minimum.reduce(abs(x))
    else:
        return Num.sum(abs(x)**ord)**(1.0/ord)
...
def rosen_der(x):
    ...
    xm_m1 = x[:-2]
    xm_p1 = x[2:]
    der = zeros(x.shape,x.dtype.char)
...
def fmin(func, x0, args=(), xtol=1e-4, ftol=1e-4, maxiter=None, maxfun=None,
    ...
    Additional Inputs:
      ...
      ftol -- acceptable absolute error in func(xopt) for convergence.
...
def check_grad(func, grad, x0, *args):
    return Num.sqrt(sum((grad(x0,*args)-approx_fprime(x0,func,_epsilon,*args))**2))
    ...
    while (gnorm > gtol) and (k < maxiter):
        ...
        alpha_k, fc, gc, old_fval, old_old_fval, gfkp1 = \
                     line_search(f,myfprime,xk,pk,gfk,
                                 old_fval,old_old_fval,args=args)
        #alpha_k, fc, gc, old_fval = \
        #             line_search_BFGS(f,xk,pk,gfk,
        #                         old_fval, args=args)
        #gfkp1 = None
        func_calls = func_calls + fc
        grad_calls = grad_calls + gc
        ...
        try:
            ...
            warnflag = 2
            print "Divide by zero encountered:  gnorm still %s" % gnorm
            break
        else:
...
def fminbound(func, x1, x2, args=(), xtol=1e-5, maxfun=500,
    ...
    sqrt_eps = Num.sqrt(2.2e-16)
    golden_mean = 0.5*(3.0-Num.sqrt(5.0))
    a, b = x1, x2
...
def _linesearch_powell(linesearch, func, p, xi, args=(), tol=1e-3):
    # line-search algorithm using fminbound
    #  find the minimium of the function
    #  func(x0+ alpha*direc)
    global _powell_funcalls
    extra_args = (func, p, xi) + args
    alpha_min, fret, iter, num = linesearch(_myfunc, args=extra_args,
...
def fmin_powell(func, x0, args=(), xtol=1e-4, ftol=1e-4, maxiter=None, maxfun=None,
         full_output=0, disp=1, retall=0, linesearch=brent):
    ...
    while 1:
        if disp:
            print "\tNumber of function evaluations = %d; current F = %f"  % (_powell_funcalls, fval)
        fx = fval
        bigind = 0
        delta = 0.0
        for i in ilist:
            direc1 = direc[i]
            fx2 = fval
            fval, x, direc1 = _linesearch_powell(linesearch, func, x, direc1, args=args, tol=xtol)
            if (fx2 - fval) > delta:
                delta = fx2 - fval
                bigind = i
        iter += 1
    ...
    #x = squeeze(x)  # makes bug triggering 0D array when only one parameter
    if full_output:
