...
#define PYCOGENT_VERSION "1.6.0.dev"
...
complex compl1 (double re,double im)
{
    complex r;
...
int cmatout (FILE * fout, complex x[], int n, int m)
{
   int i,j;
   for (i=0,fputc('\n',fout); i<n; i++, fputc('\n',fout))
        for(j = 0; j < m; j++) fprintf(fout, "%7.3f%7.3f  ", x[i*m+j].re, x[i*m+j].im);
   return (0);
}
      ...
      for (en = n-1; en >= 0; en--) {
         ...
         if ((q = vali[en]) < 0) {            /* complex vector */
            ...
            if (fabs(mat[pos(en,en-1,n)]) > fabs(mat[pos(en-1,en,n)])) {
               ...
               v = cdiv(compl1(0.0,-mat[pos(en-1,en,n)]),
                    compl1(mat[pos(en-1,en-1,n)]-p,q));
            ...
            for (i = en - 2; i >= 0; i--) {
               ...
               if (vali[i] < 0) {
                  ...
                  m = i;
                  if (vali[i] == 0) {
                     v = cdiv(compl1(-ra,-sa),compl1(w,q));
                     ...
                     v = cdiv(compl1(x*r-z*ra+q*sa,x*s-z*sa-q*ra),v);
                     mat[pos(i,en-1,n)] = v.re;
                     mat[pos(i,en,n)] = v.im;
                     if (fabs(x) > fabs(z) + fabs(q)) {
                        ...
                        v = cdiv(compl1(-r-y*mat[pos(i,en-1,n)],
                             -s-y*mat[pos(i,en,n)]),compl1(z,q));
